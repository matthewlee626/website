...
const areas: Record<string, string> = { "center": "Center", south: "South", east: "East", north: "North", west: "West", chaoyang: "Chaoyang", wangjing: "Wangjing", haidian: "Haidian", misc: "Beyond the city" };
...
const bulletPlaces: Record<string, (string | null)[]> = {
  "center": ["1", "2", "3"], south: ["4", "5"], east: ["6", "7", "8", "9", null],
...
export default function BeijingReader() {
  return <TravelReader config={beijingMap}>{(selectLocation, locationId) => <>
      ...
      <section id="history" data-map-area="all">
        <h2>history</h2>
        {article.history.map((paragraph, index) => <p key={index}>{inline(paragraph)}</p>)}
      </section>
      <section id="food" data-map-area="all">
        <h2>food</h2>
        {article.food.map((paragraph, index) => <p key={index}>{inline(paragraph)}</p>)}
      </section>
